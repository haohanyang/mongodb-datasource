  ...
  stages: [
    {
      name: '$addFields',
      description:
        'Adds new fields to documents. Similar to $project, $addFields reshapes each document in the stream; specifically, by adding new fields to output documents that contain both the existing fields from the input documents and the newly added fields. $set is an alias for $addFields.',
    },
    {
      name: '$bucket',
      description:
        'Categorizes incoming documents into groups, called buckets, based on a specified expression and bucket boundaries.',
      fields: ['groupBy', 'boundaries', 'default', 'output'],
    },
    {
      name: '$bucketAuto',
      description:
        'Categorizes incoming documents into a specific number of groups, called buckets, based on a specified expression. Bucket boundaries are automatically determined in an attempt to evenly distribute the documents into the specified number of buckets.',
      fields: ['groupBy', 'buckets', 'output', 'granularity'],
    },
    {
      name: '$changeStream',
      description:
    ...
    },
    {
      ...
      description:
        'Splits large change stream events that exceed 16 MB into smaller fragments returned in a change stream cursor. You can only use $changeStreamSplitLargeEvent in a $changeStream pipeline and it must be the final stage in the pipeline.',
    },
    ...
    { name: '$indexStats', description: 'Returns statistics regarding the use of each index for the collection.' },
    {
      ...
      insertText: '"\$limit": ${1:positive 64-bit integer}',
      fields: 'positive 64-bit integer',
    },
    {
    ...
    },
    {
      name: '$queryStats',
      description:
        "Returns runtime statistics for recorded queries. WARNING: The $queryStats aggregation stage is unsupported and is not guaranteed to be stable in a future release. Don't build functionality that relies on a specific output format of this stage, since the output may change in a future release.",
    },
    {
      name: '$redact',
      description:
    ...
    },
    {
      name: '$replaceRoot',
      description:
        'Replaces a document with the specified embedded document. The operation replaces all existing fields in the input document, including the _id field. Specify a document embedded in the input document to promote the embedded document to the top level. $replaceWith is an alias for $replaceRoot stage.',
    },
    {
      name: '$replaceWith',
      description:
    ...
    { name: '$sample', description: 'Randomly selects the specified number of documents from its input.' },
    {
      name: '$search',
      description:
        'Performs a full-text search of the field or fields in an Atlas collection. $search is only available for MongoDB Atlas clusters, and is not available for self-managed deployments. To learn more, see Atlas Search Aggregation Pipeline Stages.',
    },
    {
      name: '$searchMeta',
      description:
        'Returns different types of metadata result documents for the Atlas Search query against an Atlas collection. $searchMeta is only available for MongoDB Atlas clusters, and is not available for self-managed deployments. To learn more, see Atlas Search Aggregation Pipeline Stages.',
    },
    {
      name: '$set',
      description:
    ...
    },
    {
      name: '$vectorSearch',
      description:
        'Performs an ANN or ENN search on a vector in the specified field of an Atlas collection. $vectorSearch is only available for MongoDB Atlas clusters running MongoDB v6.0.11 or higher, and is not available for self-managed deployments. New in version 7.0.2.',
    },
  ],
};

module.exports = {
  generateCompletionData: function () {
    const filePath = path.join('src', 'editor', 'completions.json');
    return fs.writeFile(filePath, JSON.stringify(completions, null, 2));
  },
};
